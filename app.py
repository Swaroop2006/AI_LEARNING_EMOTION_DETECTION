...
EMOTION_COLORS = {
    "Confused":   "#A855F7", "Frustrated": "#F43F5E", "Curious":   "#22D3EE",
    "Confident":  "#10B981", "Excited":    "#EC4899", "Anxious":   "#FB923C",
    "Neutral":    "#FACC15",
}
EMOTION_EMOJI = {
...
html, body, .stApp {
    background: radial-gradient(1100px 700px at 8% -8%,  rgba(168,85,247,.20) 0%, transparent 55%),
                radial-gradient(1000px 700px at 95% 0%,  rgba(236,72,153,.16) 0%, transparent 50%),
                radial-gradient(900px 600px at 50% 105%, rgba(251,146,60,.12) 0%, transparent 55%),
                var(--bg-0);
    color: var(--text-hi);
}
...
.title {
    font-size: 2.6rem; font-weight: 900; letter-spacing:-.03em; line-height:1.1;
    background: linear-gradient(90deg, var(--purple) 0%, var(--pink) 50%, var(--orange) 100%);
    -webkit-background-clip: text; background-clip: text; color: transparent;
    margin: 0;
}
.subtitle { color: var(--text-mid); margin: .5rem 0 1.3rem 0; font-size: 1.02rem; }

.section-head { display:flex; align-items:center; gap:.6rem; margin: .3rem 0 .9rem 0; }
.section-head .bar { width:5px; height:1.5rem; border-radius:4px; }
.section-head span {
...
/* ---- pills / badges ---------------------------------------------------*/
.pill-row { display:flex; gap:.55rem; flex-wrap:wrap; margin: .3rem 0 .2rem 0; }
.pill {
    display:inline-flex; align-items:center; gap:.4rem;
    background: var(--surface); border:1px solid var(--border);
    border-radius: 999px; padding: .32rem .85rem; font-size:.8rem;
    font-weight:700; color: var(--text-mid); backdrop-filter: blur(6px);
}
.pill .dot { width:.5rem; height:.5rem; border-radius:50%; display:inline-block; }
.dot-on   { background: var(--good); box-shadow: 0 0 8px var(--good); }
.dot-off  { background: #5C5878; }

.badge    { display:inline-block; border-radius:999px; padding:2px 10px;
            font-size:.74rem; font-weight:700; margin-left:4px; }
.badge-ok    { background:rgba(16,185,129,.18); color:#34D399; border:1px solid rgba(16,185,129,.4); }
.badge-mixed { background:rgba(236,72,153,.18); color:#F9A8D4; border:1px solid rgba(236,72,153,.4); }

/* ---- cards -------------------------------------------------------------*/
.card {
    position:relative; background: var(--surface); border:1px solid var(--border);
    border-radius: 18px; padding: 1.3rem 1.5rem 1.3rem 1.6rem; margin-bottom: .9rem;
...
.card-title { font-weight:800; color: var(--text-mid); font-size:.82rem;
              text-transform:uppercase; letter-spacing:.08em; margin-bottom:.4rem; }
.emotion-line { font-size:1.7rem; font-weight:800; }
.conf-line { color: var(--text-lo); font-size:.85rem; margin-top:.2rem; }

.final-card {
    position:relative; overflow:hidden;
    background: linear-gradient(135deg, rgba(168,85,247,.14), rgba(236,72,153,.10) 50%, rgba(251,146,60,.08));
    border: 1px solid rgba(255,255,255,.12);
    border-radius: 20px; padding: 1.6rem 1.8rem; margin: .5rem 0 1.1rem 0;
    backdrop-filter: blur(10px); box-shadow: 0 14px 34px rgba(0,0,0,.4);
}

.guidance-box {
    position:relative; background: var(--surface); border:1px solid var(--border);
    border-radius: 16px; padding: 1.5rem 1.7rem 1.5rem 1.9rem; margin-top:.7rem;
    line-height: 1.65; color: var(--text-hi);
    box-shadow: 0 10px 28px rgba(0,0,0,.3); overflow:hidden;
}
.guidance-box::before {
    content:""; position:absolute; left:0; top:0; bottom:0; width:4px;
...
.setup-box {
    background: var(--surface); border: 1px solid rgba(251,146,60,.45);
    border-radius: 20px; padding: 1.8rem 2.1rem; backdrop-filter: blur(10px);
}

/* ---- streamlit control overrides ---------------------------------------*/
.stTextArea textarea {
    background: var(--surface) !important; border:1px solid var(--border) !important;
    border-radius: 14px !important; color: var(--text-hi) !important;
}
.stTextArea textarea:focus {
    border-color: var(--pink) !important;
    box-shadow: 0 0 0 3px rgba(236,72,153,.22) !important;
}
...
.stButton > button:hover {
    transform: translateY(-1px); border-color: var(--pink) !important;
...
.stButton > button[kind="primary"] {
    ...
    border: none !important; color: #0A0710 !important;
}
.stButton > button[kind="primary"]:hover { background-position: 100% 0 !important; }

div[data-testid="stMetric"] {
    position:relative; overflow:hidden;
    background: var(--surface); border:1px solid var(--border);
    border-radius: 16px; padding: 1rem 1.1rem .8rem 1.3rem;
...
div[data-testid="stMetricLabel"] { color: var(--text-mid); }
div[data-testid="stMetricValue"] {
    background: linear-gradient(90deg, var(--text-hi), var(--pink));
    -webkit-background-clip: text; background-clip: text; color: transparent;
}

hr { border-color: var(--border) !important; opacity: .6; margin: 1.6rem 0 !important; }

.stTabs [data-baseweb="tab-list"] { gap: .3rem; }
.stTabs [data-baseweb="tab"] { color: var(--text-mid); font-weight:700; font-size:1rem; }
.stTabs [aria-selected="true"] {
    color: var(--text-hi) !important;
    background: linear-gradient(90deg, rgba(168,85,247,.15), rgba(236,72,153,.15)) !important;
...
def _section_head(icon: str, label: str, c1: str = "var(--purple)", c2: str = "var(--pink)") -> None:
    grad = f"linear-gradient(90deg, {c1}, {c2})"
    st.markdown(
        f'<div class="section-head">'
        f'<span class="bar" style="background:{grad}"></span>'
        f'<span style="background:{grad};background-clip:text;-webkit-background-clip:text">'
...
# HEADER
# ══════════════════════════════════════════════════════════════════════════
st.markdown('<div class="eyebrow">Emotion AI · Learning Companion</div>', unsafe_allow_html=True)
...
# ── Status pills (only shown when models are loaded) ────────────────────────
gemini_dot = "dot-on" if gemini_ready else "dot-off"
gemini_lbl = "Gemini connected" if gemini_ready else "Gemini — no key"
st.markdown(
    '<div class="pill-row">'
    '<span class="pill"><span class="dot dot-on"></span>BiLSTM ready</span>'
    '<span class="pill"><span class="dot dot-on"></span>DistilBERT ready</span>'
    f'<span class="pill"><span class="dot {gemini_dot}"></span>{gemini_lbl}</span>'
    '</div>',
    unsafe_allow_html=True,
)
...
# ══════════════════════════════════════════════════════════════════════════
tab_analyze, tab_analytics = st.tabs(["🔍  Analyze", "📈  Analytics"])

with tab_analyze:
    # ── INPUT PANEL ──────────────────────────────────────────────────────
    _section_head("📝", "Input", "var(--purple)", "var(--pink)")
    ...
    if st.session_state.result is not None:
        ...
        st.divider()
        _section_head("📊", "Results", "var(--pink)", "var(--orange)")
        ...
        for col, key, title in [(mc1, "bilstm", "BiLSTM"), (mc2, "bert", "DistilBERT")]:
            ...
            with col:
                ...
                st.markdown(
                    ...
                    f'<span class="emotion-line" style="color:{color}">{emoji} {r["primary"]}</span>'
                    f'<div class="conf-line">{r["primary_confidence"]:.1%} confidence{mixed_html}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
        ...
        st.markdown(
            f'<div class="final-card" style="border:1px solid {f_color}66;">'
            f'<div class="card-title">🏆 Final Prediction</div>'
            f'<span class="emotion-line" style="color:{f_color}">{f_emoji} {final["primary"]}</span>'
            f'&nbsp;<span style="color:var(--text-lo);font-size:.9rem">'
        ...
        # ── Guidance panel ───────────────────────────────────────────────
        st.divider()
        _section_head("💬", "AI Guidance", "var(--orange)", "var(--purple)")
        ...
        st.info("Enter a message above and click **Analyse** to see results here.")

# ══════════════════════════════════════════════════════════════════════════
# ANALYTICS DASHBOARD
# ══════════════════════════════════════════════════════════════════════════
with tab_analytics:
    _section_head("📈", "Analytics", "var(--purple)", "var(--orange)")
    ...
    with ch1:
        if total > 0 and "final_primary" in df.columns:
            ...
            fig_donut.update_layout(title="Emotion Distribution")
    ...
    with ch2:
        if total > 0 and "final_conf" in df.columns:
            ...
            fig_trend.update_layout(title="Confidence Over Time")
        _style_fig(fig_trend)
        with st.container(border=True):
